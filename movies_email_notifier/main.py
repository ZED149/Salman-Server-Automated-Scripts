...
# then, it will send an email to the receipents(defined in a seperate Excel file) regarding what folders are added to the ROOT
...
# loading our ENVIORMENTAL VARIABLES into our scope
load_dotenv()
...
# DB_Fetch_Names
def DB_Fetch_Names() -> list:
    # making a connection to the DB
    try:
        conn = sqlite3.connect(DB_NAME)
    except:
        # send an email to the server admin regarding failure to connect to the movies DB
        exit(0)
    cursor = conn.cursor()
...
""")
    ...
    # appending list of movies to the mmessage
...
# need to send an email to the receipents regarding the name of the new movies
